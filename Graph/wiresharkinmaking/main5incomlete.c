...
#include <stdlib.h>
#include <ctype.h>
#include <errno.h>//macros for errors reporting
#include <sys/types.h>
#include <sys/socket.h>
#include <netinet/in.h>
#include <arpa/inet.h>
#include "soclib.c"
Socket_record *soct;//,*socu,*soci;
...
void got_packet(u_char *args, const struct pcap_pkthdr *header, const u_char *packet){
	...
	//struct icmphdr *icphdr,dhcp;
	const u_char *payload; 
	static int size_ip,ip_count=0;
	static int size_tcp,tcp_count=0;
	static int size_udp,udp_count=0;
	...
	switch(ip->ip_p) {
		...
		case IPPROTO_UDP:
			...
			if(size_payload>0)
			{
				...
				if(ntohs(udp->uh_sport)==67)print_payload(payload,size_payload);
			}
			return;
		case IPPROTO_ICMP:
			//icmp_count++;
...
int main(int argc, char **argv){
	int tcpsoc=8000;//udpsoc=8001,icmpsoc=8002;
	//system("/home/jindal/Desktop/'Network analyzer'/Graph/XGraph4.30_linux64/bin/xgraph -x_range 0 100 -y_range 0 100 -soc 8000");
	soct = Setup_Client_Socket( "", tcpsoc );//"" is localhost or use an ip
	/*sleep(1);
	socu = Setup_Client_Socket( "", udpsoc );
	sleep(1);
	soci = Setup_Client_Socket( "", icmpsoc );
	sleep(1);*/
