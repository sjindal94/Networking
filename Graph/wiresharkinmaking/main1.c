...
//#include<freeradius-dlevel/libradius.h>
//#include<freeradius-dlevel/udpfromoto.h>
//#include<freeradius-dlevel/dhcp.h>
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
#include <unistd.h>
...
int soc_num;
clock_t begin, end;
static int tcp_num,ip_num,udp_num,icp_num,dhcp_num,l;
FILE *tp,*up,*ipp,*icp;
...
void got_packet(u_char *args,const struct pcap_pkthdr *header,const u_char *packet)
{
	...
	/*case IPPROTO_DHCP:
	    ...
	    dhcp_num++;
            ...
            seconds_since_start =(float)(end-begin) ;
            sprintf( message, "%f %d\n",seconds_since_start/100, dhcp_num );
	    Send_Socket( soc, message );
	    write(1,message,strlen(message));
            f=5;
            break;
   ...
   /* else if(f==5)
    {
        ...
        struct sniff_dhcp *dhcphdr=(struct sniff_dhcp*)(packet+SIZE_ETHERNET+size_ip);
	int size = offset(dhcphdr,options);
        payload=(u_char*)(packet+SIZE_ETHERNET+size_ip);
        size_payload=ntohs(ip->ip_len)-(size_ip);
        printf(" Payload Size :%d\n",size_payload);
    }*/
...
{
   // while(1)
//   execlp("/home/lashkeretrishul/Desktop/nt/graph","graph",(char*)0);
soc_num=atoi(argv[1]);
soc = Setup_Client_Socket( "", 8000);
//run this first
//xgraph -x_range 0 200  -y_range 1 100  -soc 8000
    {
        begin=clock();
       /* tp=fopen("tcp.txt","w");
        up=fopen("udp.txt","w");
      ...
      /*  else
        ...
        fclose(up);
        Close_Socket( soc );
 //       system("xgraph tcp.txt ip.txt ");system("xgraph udp.txt icmp.txt");
    }
}
